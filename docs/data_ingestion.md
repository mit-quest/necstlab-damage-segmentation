...
Note:
- `ingest_raw_data.py` assumes that if `dmg` appears in the zip filename that the zip file is has annotations. If no `dmg` appears it assumes it contains images.
